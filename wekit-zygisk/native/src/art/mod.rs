...
use jni::sys::{
    jclass, jfieldID, jmethodID, jobject,
    JNIEnv as RawJNIEnv, JNI_FALSE,
};
use libc::c_int;
...
impl ScopedArtSuspend {
    unsafe fn new(reason: &str) -> Self {
        let mut s = Self { storage: [0u8; 256] };
        ...
        if ctor_fn != 0 {
            ...
            ctor(s.storage.as_mut_ptr(), reason_c.as_ptr() as *const u8, false);
        }
        s
    }
    fn active(&self) -> bool {
        G_SUSPEND_CTOR.load(Ordering::Relaxed) != 0
            && G_SUSPEND_DTOR.load(Ordering::Relaxed) != 0
    }
}
...
impl WritableArtMethod {
    unsafe fn acquire(addr: usize, method_size: usize) -> Option<Self> {
        if addr == 0 || method_size == 0 { return None; }
        let page_size = libc::sysconf(libc::_SC_PAGESIZE) as usize;
        let start = addr & !(page_size - 1);
        let end = ((addr + method_size - 1) & !(page_size - 1)) + page_size;
        let len = end - start;
        let orig_prot = get_prot_for_addr(addr)?;
        let changed = orig_prot & libc::PROT_WRITE == 0;
        if changed
            && libc::mprotect(start as *mut _, len,
                orig_prot | libc::PROT_WRITE) < 0 {
                loge!("Zygisk: mprotect writable failed for {:#x}: {}",
                    addr, std::io::Error::last_os_error());
                return None;
            }
        Some(WritableArtMethod { page_start: start, page_len: len, orig_prot, changed })
    }
}
...
unsafe fn init_reflection_fields(env: *mut RawJNIEnv) -> bool {
    ...
    let art_method_fid = ((*fns).v1_6.GetFieldID)(env, exec_cls, c"artMethod".as_ptr(), c"J".as_ptr());
    ...
    let access_flags_fid = ((*fns).v1_6.GetFieldID)(env, exec_cls, c"accessFlags".as_ptr(), c"I".as_ptr());
...
unsafe fn probe_art_method_layout(env: *mut RawJNIEnv)
    -> Option<(usize, usize, u32)>  // (first_art_method, method_size, access_flags)
{
    let fns = *env;
    ...
    if throwable.is_null() || clazz.is_null() {
        ((*fns).v1_6.ExceptionClear)(env);
        if !throwable.is_null() { ((*fns).v1_6.DeleteLocalRef)(env, throwable); }
        if !clazz.is_null()     { ((*fns).v1_6.DeleteLocalRef)(env, clazz); }
        return None;
    }
    let get_ctors = ((*fns).v1_6.GetMethodID)(env, clazz,
    ...
    if ctors.is_null() || ((*fns).v1_6.ExceptionCheck)(env) != JNI_FALSE {
        ((*fns).v1_6.ExceptionClear)(env);
        if !ctors.is_null() { ((*fns).v1_6.DeleteLocalRef)(env, ctors); }
        return None;
    }
    ...
    if !c0.is_null() { ((*fns).v1_6.DeleteLocalRef)(env, c0); }
    if !c1.is_null() { ((*fns).v1_6.DeleteLocalRef)(env, c1); }
    ((*fns).v1_6.DeleteLocalRef)(env, ctors);
    ((*fns).v1_6.ExceptionClear)(env);
    if first == 0 || second == 0 { return None; }
    let size = first.abs_diff(second);
    // 合理范围检查
    if size < std::mem::size_of::<usize>() * 3 || size > 256 || size % std::mem::size_of::<usize>() != 0 {
...
unsafe fn find_access_flags_offset(art_method: usize, method_size: usize, reflected_flags: u32)
    -> Option<usize>
{
    ...
    while candidate + 4 <= method_size {
        ...
        if value == reflected_flags {
            if candidate == 4 {  // preferred position (after declaring_class GC root)
                return Some(candidate);
            }
            if found.is_none() { found = Some(candidate); }
        }
        candidate += 4;
    }
    found
...
unsafe fn resolve_dex_file_set_trusted_method(env: *mut RawJNIEnv) -> jmethodID {
    ...
    let mid = ((*fns).v1_6.GetStaticMethodID)(env, cls,
        c"setTrusted".as_ptr(),
        c"(Ljava/lang/Object;)V".as_ptr());
    if mid.is_null() { ((*fns).v1_6.ExceptionClear)(env); }
...
pub fn init(env: *mut RawJNIEnv) -> bool {
    ...
    let (first_art_method, method_size, sample_flags) =
        match unsafe { probe_art_method_layout(env) } {
            Some(v) => v,
            None => { loge!("Zygisk: failed to probe ART method layout"); return false; }
        };
    ...
    let access_flags_offset =
        match unsafe { find_access_flags_offset(first_art_method, method_size, sample_flags) } {
            Some(o) => o,
            None => { loge!("Zygisk: failed to find access_flags offset"); return false; }
        };
    ...
    if access_flags_offset + 4 > entry_point_offset {
        loge!("Zygisk: invalid layout: flags={} entry={} size={}",
            access_flags_offset, entry_point_offset, method_size);
        return false;
    }
    ...
    let suspend_ctor = {
        ...
        if v != 0 { v } else { resolve("_ZN3art16ScopedSuspendAllC1EPKcb", false) }
    };
    let suspend_dtor = {
        let v = resolve("_ZN3art16ScopedSuspendAllD2Ev", false);
        if v != 0 { v } else { resolve("_ZN3art16ScopedSuspendAllD1Ev", false) }
    };
    ...
    let mut set_trusted = resolve("_ZN3artL18DexFile_setTrustedEP7_JNIEnvP7_jclassP8_jobject", true);
    if set_trusted == 0 {
        // 尝试 .gnu_debugdata
        if let Some(off) = find_symbol_in_file(&art.path, "_ZN3artL18DexFile_setTrustedEP7_JNIEnvP7_jclassP8_jobject") {
    ...
    if suspend_ctor == 0 || suspend_dtor == 0
        || (set_trusted == 0 && set_trusted_method == 0)
    {
        loge!("Zygisk: required ART entry missing: ctor={:#x} dtor={:#x} trusted_sym={:#x} trusted_mid={:#x}",
    ...
    unsafe extern "C" { fn android_get_device_api_level() -> libc::c_int; }
    let api = unsafe { android_get_device_api_level() };
    let acc_precompiled: u32 = if api < 30 { 0 } else if api >= 31 { 0x00800000 } else { 0x00200000 };
    ...
    *G_HOOK_RECORDS.lock().unwrap() = Some(HashMap::new());
    ...
    logi!("Zygisk: ART layout: method_size={} entry_offset={} access_flags_offset={} api={}",
        method_size, entry_point_offset, access_flags_offset, api);
    true
}
...
pub fn hook_method(_env: *mut RawJNIEnv, target_art: usize, backup_art: usize, bridge_art: usize) -> i32 {
    if !is_initialized() || target_art == 0 || backup_art == 0 || bridge_art == 0 { return -1; }
    ...
    unsafe {
        // 检查是否已 hook
        if G_HOOK_RECORDS.lock().unwrap().as_ref()
                .is_some_and(|m| m.contains_key(&target_art)) {
            loge!("Zygisk: target={:#x} already has an active hook", target_art);
            return -3;
        }

        let _tw  = match WritableArtMethod::acquire(target_art, method_size) { Some(w) => w, None => return -4 };
        let _bw  = match WritableArtMethod::acquire(backup_art, method_size) { Some(w) => w, None => return -5 };
        let _brw = match WritableArtMethod::acquire(bridge_art, method_size) { Some(w) => w, None => return -6 };
        ...
        // 分配并写入 trampoline
        let pool = match G_POOL_STORAGE.get() { Some(p) => p, None => return -7 };
        ...
        G_HOOK_RECORDS.lock().unwrap().as_mut().unwrap()
            .insert(target_art, HookRecord { backup_art, original_access_flags });
    }
    logi!("Zygisk: hooked target={:#x} bridge={:#x}", target_art, bridge_art);
    0
}
...
pub fn unhook_method(_env: *mut RawJNIEnv, target_art: usize, backup_art: usize) -> i32 {
    if !is_initialized() || target_art == 0 || backup_art == 0 { return -1; }
    ...
    let original_access_flags = {
        ...
        match records.as_ref().and_then(|m| m.get(&target_art)) {
            ...
            _ => {
                loge!("Zygisk: unhook: target={:#x} has no matching active hook", target_art);
                return -2;
            }
        }
    };
    ...
    unsafe {
        ...
        // 写回原始 access_flags（backup 里的已被运行时修改过）
        ((target_art + af_off) as *mut u32).write_volatile(original_access_flags);
    }
    G_HOOK_RECORDS.lock().unwrap().as_mut().unwrap().remove(&target_art);
...
pub fn trust_dex_file(env: *mut RawJNIEnv, dex_file: jobject) -> bool {
    ...
    unsafe {
        ...
        // 获取 mCookie 字段
        let cookie_fid = ((*fns).v1_6.GetFieldID)(env, dex_cls,
            c"mCookie".as_ptr(), c"Ljava/lang/Object;".as_ptr());
        ...
        if ((*fns).v1_6.ExceptionCheck)(env) != JNI_FALSE || cookie.is_null() {
            ((*fns).v1_6.ExceptionClear)(env);
            if !cookie.is_null() { ((*fns).v1_6.DeleteLocalRef)(env, cookie); }
            ((*fns).v1_6.DeleteLocalRef)(env, dex_cls);
...
pub fn trust_class_loader(env: *mut RawJNIEnv, class_loader: jobject) -> bool {
    ...
    unsafe {
        ...
        if base_cls.is_null() || plist_cls.is_null() || elem_cls.is_null() {
            ((*fns).v1_6.ExceptionClear)(env);
            for p in [base_cls, plist_cls, elem_cls].iter().filter(|&&p| !p.is_null()) {
                ((*fns).v1_6.DeleteLocalRef)(env, *p);
            }
            return false;
        }
        let plist_fid = ((*fns).v1_6.GetFieldID)(env, base_cls,
            c"pathList".as_ptr(), c"Ldalvik/system/DexPathList;".as_ptr());
        let elems_fid = ((*fns).v1_6.GetFieldID)(env, plist_cls,
            c"dexElements".as_ptr(), c"[Ldalvik/system/DexPathList$Element;".as_ptr());
        let dexfile_fid = ((*fns).v1_6.GetFieldID)(env, elem_cls,
            c"dexFile".as_ptr(), c"Ldalvik/system/DexFile;".as_ptr());
        ...
        if ((*fns).v1_6.ExceptionCheck)(env) != JNI_FALSE || elements.is_null() {
            ((*fns).v1_6.ExceptionClear)(env);
            if !path_list.is_null() { ((*fns).v1_6.DeleteLocalRef)(env, path_list); }
            if !elements.is_null()  { ((*fns).v1_6.DeleteLocalRef)(env, elements as jobject); }
            for p in [base_cls, plist_cls, elem_cls] { ((*fns).v1_6.DeleteLocalRef)(env, p); }
            return false;
        }
        ...
        for i in 0..count {
            ...
            if ((*fns).v1_6.ExceptionCheck)(env) != JNI_FALSE {
                ...
                if trust_dex_file(env, dex_file) { trusted_count += 1; }
                else { success = false; }
            }
            if !dex_file.is_null() { ((*fns).v1_6.DeleteLocalRef)(env, dex_file); }
            if !elem.is_null()     { ((*fns).v1_6.DeleteLocalRef)(env, elem); }
        }
        ((*fns).v1_6.DeleteLocalRef)(env, elements as jobject);
        if !path_list.is_null() { ((*fns).v1_6.DeleteLocalRef)(env, path_list); }
        for p in [base_cls, plist_cls, elem_cls] { ((*fns).v1_6.DeleteLocalRef)(env, p); }
        if !success || trusted_count == 0 {
            loge!("Zygisk: failed to trust every DexFile in class loader (trusted={})", trusted_count);
            return false;
        }
        logi!("Zygisk: trusted {} DexFile(s) for class loader {:p}", trusted_count, class_loader);
        true
    }
}
