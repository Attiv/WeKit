...
unsafe fn get_class_loader(env: *mut RawJNIEnv, entry_class: jclass) -> jobject {
    ...
    if class_cls.is_null() { ((*fns).v1_6.ExceptionClear)(env); return std::ptr::null_mut(); }
    let mid = ((*fns).v1_6.GetMethodID)(env, class_cls,
        c"getClassLoader".as_ptr(), c"()Ljava/lang/ClassLoader;".as_ptr());
    ((*fns).v1_6.DeleteLocalRef)(env, class_cls);
    if mid.is_null() { ((*fns).v1_6.ExceptionClear)(env); return std::ptr::null_mut(); }
...
extern "C" fn jni_native_initialize(env: *mut RawJNIEnv, entry: jclass) -> jboolean {
    unsafe {
        ...
        if loader.is_null() { return JNI_FALSE; }
        ...
        if !ok { loge!("Zygisk: ZygiskEntry.nativeInitialize: failed to register ArtHookBridge"); }
        ((*(*env)).v1_6.DeleteLocalRef)(env, loader);
        if ok { JNI_TRUE } else { JNI_FALSE }
    }
}
...
unsafe fn connect_telegram_socket() -> libc::c_int {
    ...
    if nb.len() >= 107 { libc::close(fd); return -1; }
    addr.sun_path[0] = 0;
    for (i, &b) in nb.iter().enumerate() { addr.sun_path[1 + i] = b as libc::c_char; }
...
unsafe fn receive_snapshot_file(sock: libc::c_int, dst_fd: libc::c_int) -> Result<u64, ()> {
    ...
    if size > 1024 * 1024 * 1024 { return Err(()); }
    if libc::ftruncate(dst_fd, 0) != 0 { return Err(()); }
    if libc::lseek(dst_fd, 0, libc::SEEK_SET) < 0 { return Err(()); }
...
extern "C" fn jni_has_telegram_root_companion(_env: *mut RawJNIEnv, _class: jclass) -> jboolean {
    if crate::TELEGRAM_SOCKET_NAME.lock().unwrap().is_empty() { JNI_FALSE } else { JNI_TRUE }
}
...
extern "C" fn jni_list_telegram_instances(env: *mut RawJNIEnv, _class: jclass) -> jobject {
    unsafe {
        ...
        if sock < 0 { throw_telegram_error(env, "Telegram Root companion is unavailable"); return std::ptr::null_mut(); }
        ...
        if let Err(e) = receive_telegram_response(sock) {
            libc::close(sock); throw_telegram_error(env, &e); return std::ptr::null_mut();
        }
        let count = match crate::protocol::read_u16_from_fd(sock) {
            Ok(n) if n > 0 && n <= 64 => n,
            _ => { libc::close(sock); throw_telegram_error(env, "Invalid Telegram instance list from root companion"); return std::ptr::null_mut(); }
        };
        let mut packages: Vec<String> = Vec::new();
        for _ in 0..count {
            match crate::protocol::read_string_from_fd(sock) {
                Ok(p) if is_valid_telegram_package(&p) => packages.push(p),
                _ => { libc::close(sock); throw_telegram_error(env, "Invalid Telegram package from root companion"); return std::ptr::null_mut(); }
            }
        }
        libc::close(sock);
        let fns = *env;
        let str_cls = ((*fns).v1_6.FindClass)(env, c"java/lang/String".as_ptr());
        if str_cls.is_null() { return std::ptr::null_mut(); }
        let arr = ((*fns).v1_6.NewObjectArray)(env, packages.len() as jint, str_cls, std::ptr::null_mut());
        ...
        for (i, pkg) in packages.iter().enumerate() {
            let Ok(c_pkg) = std::ffi::CString::new(pkg.as_str()) else {
                ((*fns).v1_6.DeleteLocalRef)(env, arr as jobject); return std::ptr::null_mut();
            };
            let jstr = ((*fns).v1_6.NewStringUTF)(env, c_pkg.as_ptr());
            if jstr.is_null() { ((*fns).v1_6.DeleteLocalRef)(env, arr as jobject); return std::ptr::null_mut(); }
...
extern "C" fn jni_copy_telegram_database_snapshot(
    ...
    unsafe {
        ...
        if chars.is_null() { return 0; }
        let pkg = std::ffi::CStr::from_ptr(chars).to_string_lossy().into_owned();
        ((*fns).v1_6.ReleaseStringUTFChars)(env, package_name, chars);
        if !is_valid_telegram_package(&pkg) || database_fd < 0 || wal_fd < 0 || shm_fd < 0 {
            throw_telegram_error(env, "Invalid Telegram database snapshot request"); return 0;
        }
        let name = crate::TELEGRAM_SOCKET_NAME.lock().unwrap().clone();
        if name.is_empty() { throw_telegram_error(env, "Telegram Root companion is unavailable"); return 0; }
        let sock = connect_telegram_socket();
        if sock < 0 { throw_telegram_error(env, "Telegram Root companion is unavailable"); return 0; }
        let _ = crate::protocol::write_u8_to_fd(sock, crate::protocol::TELEGRAM_REQUEST_COPY_DATABASE);
        let _ = crate::protocol::write_string_to_fd(sock, &pkg);
        if let Err(e) = receive_telegram_response(sock) {
            libc::close(sock); throw_telegram_error(env, &e); return 0;
        }
        ...
        libc::close(sock);
        if db_size == 0 { throw_telegram_error(env, "Telegram database snapshot transfer failed"); return 0; }
